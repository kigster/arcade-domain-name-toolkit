...
class DomainMonitor:
    ...
    def get_alerts(self) -> List[Dict]:
        ...
        for result in self.results:
            ...
            domain_config = next(
                (d for d in self.config.domains if d.name == domain_name), None
            )
            threshold = (
                domain_config.alert_threshold_days
            ...
            if (
                domain_check.get("status") == "success"
                and domain_check.get("days_until_expiry", 999) <= threshold
            ):
                alerts.append(
                    {
                        "domain": domain_name,
            ...
            if (
                ssl_check.get("status") == "success"
                and ssl_check.get("days_until_expiry", 999) <= threshold
            ):
                alerts.append(
                    {
                        "domain": domain_name,
    ...
    def send_email_alert(self, alerts: List[Dict]):
        ...
        # Build email content
        subject = self.config.notifications.email.subject_template.format(
            count=len(alerts)
        )
        ...
        body_lines = [
            ...
            "=" * 50,
            "",
            "The following domains are expiring within their alert thresholds:",
            "",
        ]
        ...
        for recipient in self.config.notifications.email.recipients:
            try:
                self.client.tools.execute(
                    ...
                    input={"to": recipient.email, "subject": subject, "body": body},
                    user_id=self.config.monitoring.user_id,
                )
    ...
    def send_slack_alert(self, alerts: List[Dict]):
        ...
        for alert in alerts:
            ...
            if days <= 7:
                emoji = self.config.notifications.slack.urgency_emojis.get(
                    "critical", "🔴"
                )
            elif days <= 30:
                emoji = self.config.notifications.slack.urgency_emojis.get(
            ...
            message_lines.append(
                f"{emoji} *{domain}* ({alert_type}) - {days} days left"
            )
        ...
        try:
            ...
            print(
                f"💬 Alert sent to Slack channel {self.config.notifications.slack.channel}"
            )
        except Exception as e:
    ...
    def run(self):
        ...
        if alerts:
            ...
            for alert in alerts:
                print(
                    f"   - {alert['domain']} ({alert['type']}) - {alert['days_until_expiry']} days"
                )
