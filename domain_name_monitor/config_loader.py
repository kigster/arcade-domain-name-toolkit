...
class EmailConfig:
    ...
    subject_template: str = "🚨 Domain Expiration Alert - {count} domain(s) expiring soon"
...
class SlackConfig:
    ...
    urgency_emojis: Dict[str, str] = field(default_factory=lambda: {
        "critical": "🔴",
        "warning": "🟡", 
        "info": "ℹ️"
    })
...
class ConfigLoader:
    ...
    def _load_yaml_config(cls, config_file: str) -> DomainMonitorConfig:
        ...
        monitoring = MonitoringConfig(
            ...
            results_filename=monitoring_data.get('results_filename', 'domain_check_results.json'),
        ...
        email_config = EmailConfig(
            ...
            subject_template=email_data.get('subject_template', 
        ...
        slack_config = SlackConfig(
            ...
            message_template=slack_data.get('message_template',
                "🚨 *Domain Alert* - {count} domain(s) expiring soon"),
            urgency_emojis=slack_data.get('urgency_emojis', {
                "critical": "🔴",
                "warning": "🟡",
                "info": "ℹ️"
            })
        ...
        advanced = AdvancedConfig(
            ...
            json_pretty_print=advanced_data.get('output', {}).get('json_pretty_print', True)
        )
    ...
    def _load_python_config(cls) -> DomainMonitorConfig:
        ...
        from domain_config import (
            DOMAINS_TO_MONITOR, USER_ID, ALERT_THRESHOLD_DAYS,
            ENABLE_EMAIL_ALERTS, EMAIL_RECIPIENTS,
            ENABLE_SLACK_ALERTS, SLACK_CHANNEL,
            SAVE_RESULTS_TO_FILE, RESULTS_FILENAME
        )
    ...
    @classmethod
    def save_config(cls, config: DomainMonitorConfig, filename: str = "domain_monitor_config.yaml"):
        ...
        data = {
            ...
            'domains': [
                {
                    ...
                    **({'alert_threshold_days': domain.alert_threshold_days} 
                       if domain.alert_threshold_days else {})
                }
            ...
            'advanced': {
                ...
                'logging': {
                    'level': config.advanced.logging_level
                },
                'output': {
