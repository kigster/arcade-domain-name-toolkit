...
def simple_domain_check():
    ...
    for domain in TEST_DOMAINS:
        ...
        # Check for alerts
        if (domain_result.get("status") == "success" and 
            domain_result.get("days_until_expiry", 999) <= ALERT_THRESHOLD_DAYS):
            alerts.append({
                "domain": domain,
        ...
        if (ssl_result.get("status") == "success" and 
            ssl_result.get("days_until_expiry", 999) <= ALERT_THRESHOLD_DAYS):
            alerts.append({
                "domain": domain,
        ...
        if domain_result.get("status") == "success":
            print(f"   🌐 Domain expires in {domain_result.get('days_until_expiry')} days")
        else:
